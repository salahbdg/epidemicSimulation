...
PROBA_DEATH = 3  # the black plague was 100%. smallpox was ~30%-35%
...
VACCINATION_RATE = 20 # percentage of vaccinated people in a population
...
def init_screen():
    ...
    image = pygame.image.load("/media/saladin/huge.disque/coding/pythonProjects/images/death_count.png").convert_alpha()
    image = pygame.transform.rotozoom(image, 0, .35)

    display.blit(image, (100, 23))


def vaccinate():
    for x in range(nb_cols):
        for y in range(nb_rows):
            if randrange(100) < VACCINATION_RATE:
                states[x][y] = 1
